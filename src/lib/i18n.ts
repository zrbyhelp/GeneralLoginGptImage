...
const exactEn: Record<string, string> = {
  ...
  '1K+ 档单张消耗': '1K+ cost per image',
  '1K+ 专用 API': '1K+ dedicated API',
  '切换 1K+ 专用 API': 'Toggle 1K+ dedicated API',
  '1K+ 专用配置': '1K+ dedicated config',
...
const regexEn: Array<[RegExp, (match: RegExpMatchArray) => string]> = [
  ...
  [/^1K\+ 专用 API，本次约消耗 (\d+) 积分$/, (m) => `1K+ dedicated API, estimated cost ${m[1]} points`],
