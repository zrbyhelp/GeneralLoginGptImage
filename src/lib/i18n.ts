...
const exactEn: Record<string, string> = {
  ...
  '如需生成 4K 图片或更高频生成，可前往商店购买卡密补充积分：5 元可兑换 50000 积分。':
    'If you need 4K images or a higher generation allowance, you can buy a top-up code from the shop to add points: 5 CNY = 50,000 points.',
  '前往商店购买卡密': 'Go to shop to buy a top-up code',
  '新版本': 'New version',
  '生成审计': 'Generation audit',
  '隐私模式': 'Privacy mode',
  '切换隐私模式': 'Toggle privacy mode',
  '是否上传图集': 'Upload to gallery',
  '上传图集': 'Upload to gallery',
  '不上传图集': 'Do not upload',
  '默认上传图集': 'Upload to gallery by default',
  '投诉建议': 'Feedback',
  '操作指南': 'Guide',
  '设置': 'Settings',
  '积分兑换': 'Redeem points',
  ...
  '关闭图集上传时每小时最多生成图片数': 'Max images per hour when gallery upload is off',
  '全服务同时生成图片数': 'Service concurrent images',
  '单账号同时生成图片数': 'Concurrent images per account',
  '每日补满积分': 'Daily refill target',
  '1K 档单张消耗': '1K cost per image',
  '2K-4K 档单张消耗': '2K-4K cost per image',
  '2K-4K 专用 API': '2K-4K dedicated API',
  '切换 2K-4K 专用 API': 'Toggle 2K-4K dedicated API',
  '2K-4K 专用配置': '2K-4K dedicated config',
  '开启后按高档位消耗积分': 'When enabled, uses high-tier points',
  '兑换码发放': 'Redeem code issuance',
  '发放数量': 'Issue quantity',
  '每个可兑换积分': 'Points per code',
  '生成并下载 TXT': 'Generate and download TXT',
...
const regexEn: Array<[RegExp, (match: RegExpMatchArray) => string]> = [
  ...
  [/^(\d+)积分$/, (m) => `${m[1]} points`],
  [/^积分：(\d+)$/, (m) => `Points: ${m[1]}`],
  [/^当前积分：(\d+)$/, (m) => `Current points: ${m[1]}`],
  [/^(.+) · (\d+) 积分$/, (m) => `${m[1]} · ${m[2]} points`],
  ...
  [/^积分不足，需要 (\d+) 积分$/, (m) => `Insufficient points, need ${m[1]}`],
  [/^积分不足，需要 (\d+) 积分，当前 (\d+) 积分$/, (m) => `Insufficient points: need ${m[1]}, current ${m[2]}`],
  [/^积分不足，点击查看补充方式$/, () => 'Insufficient points, click to view top-up options'],
  [/^本次生成需要 (\d+) 积分，当前仅剩 (\d+) 积分。\n积分会在每天自动补满至 100，也可以前往商店购买卡密补充积分：5 元可兑换 50000 积分。\n(https:\/\/pay\.ldxp\.cn\/shop\/QEJABMGR)$/, (m) => `This generation needs ${m[1]} points, but only ${m[2]} remain.\nPoints are automatically refilled to 100 every day. You can also buy a top-up code from the shop: 5 CNY = 50,000 points.\n${m[3]}`],
  [/^积分会在每天自动补满至 100，也可以前往商店购买卡密补充积分：5 元可兑换 50000 积分。$/, () => 'Points are automatically refilled to 100 every day. You can also buy a top-up code from the shop: 5 CNY = 50,000 points.'],
  [/^https:\/\/pay\.ldxp\.cn\/shop\/QEJABMGR$/, () => 'https://pay.ldxp.cn/shop/QEJABMGR'],
  [/^将提交当前提示词和参数生成图片，预计消耗 (\d+) 积分。是否继续？$/, (m) => `Submit the current prompt and settings to generate images. Estimated cost: ${m[1]} points. Continue?`],
  [/^2K-4K 专用 API，本次约消耗 (\d+) 积分$/, (m) => `2K-4K dedicated API, estimated cost ${m[1]} points`],
  [/^排队中 · 前方 (\d+) 张$/, (m) => `Queued · ${m[1]} ahead`],
  [/^fal\.ai 任务已恢复，共 (\d+) 张图片$/, (m) => `fal.ai task recovered, ${m[1]} images`],
  [/^兑换成功，获得 (\d+) 积分$/, (m) => `Redeemed successfully, gained ${m[1]} points`],
  [/^导出失败：(.+)$/, (m) => `Export failed: ${m[1]}`],
  [/^导入失败：(.+)$/, (m) => `Import failed: ${m[1]}`],
  [/^保存失败：(.+)$/, (m) => `Save failed: ${m[1]}`],
  [/^兑换失败：(.+)$/, (m) => `Redeem failed: ${m[1]}`],
  [/^兑换码发放失败：(.+)$/, (m) => `Redeem code issuance failed: ${m[1]}`],
